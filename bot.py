...
async def check_single_link(link: str, user_id: int) -> dict:
    ...
    try:
        ...
        async def _check(app: Client):
            if is_private:
                # Try get_chat first (already member)
                chat = None
                try:
                ...
                if chat is None:
                    # Try joining to get info
                    try:
                        chat = await app.join_chat(identifier)  # join_chat takes hash without +
                        await asyncio.sleep(0.5)
                        # Leave after getting info (optional — comment out if you want to stay)
                        try:
                        ...
                        # Already member — try get_chat again
                        try:
                            chat = await app.get_chat(f"+{identifier}")
                        except Exception:
                            result["status"] = "active"
                            result["title"] = "Already member (can't fetch details)"
                            return
...
async def resolve_chat_smart(app, link: str):
    ...
    if is_private:
        ...
        try:
            ...
            pass
        # Try joining — join_chat takes the hash WITHOUT '+'
