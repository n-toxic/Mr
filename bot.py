...
async def check_single_link(link: str, user_id: int) -> dict:
    ...
    try:
        ...
        async def _check(app: Client):
            if is_private:
                # Use check_invite_link to get info WITHOUT joining (Pyrogram method)
                # This is the cleanest way — no join, no leave needed
                try:
                    inv = await app.check_invite_link(identifier)
                    result["status"]  = "active"
                    result["title"]   = getattr(inv, "title", None) or "N/A"
                    result["members"] = getattr(inv, "members_count", "N/A")
                    ctype = str(getattr(inv, "type", "group")).lower()
                    result["type"]    = ctype
                    result["forward"] = "unknown"
                    return
                    ...
                    # Already member, check_invite_link still works but raises this sometimes
                    # Try join_chat to get chat object
                    pass
                except Exception:
                    # check_invite_link not available or failed — fallback to join
                    pass

                # Fallback: join to get chat info, then leave
                chat = None
                try:
                ...
                if chat is None:
                    result["status"] = "error"
                    result["title"]  = "Could not resolve chat"
                    return

                # Got chat via join_chat — fill result now and return
                result["status"]  = "active"
...
# ─────────────────────────────────────────
#  BULK LINK CHECKER — parallel (200-300 links)
# ─────────────────────────────────────────
...
async def check_links_bulk(links: list[str], user_id: int,
    ...
    batch_size=10 → 10 links at a time to avoid flood.
    """
    results = [None] * len(links)
    semaphore = asyncio.Semaphore(batch_size)

    async def _check_one(i, lnk):
        async with semaphore:
            r = await check_single_link(lnk, user_id)
            results[i] = r
            if progress_cb:
                done = sum(1 for x in results if x is not None)
                await progress_cb(done, len(links), lnk)

    tasks = [asyncio.create_task(_check_one(i, lnk)) for i, lnk in enumerate(links)]
    await asyncio.gather(*tasks, return_exceptions=True)
    # Fill any None (shouldn't happen)
    for i, r in enumerate(results):
        if r is None:
            results[i] = {"link": links[i], "status": "error", "type": "?",
...
def fmt_result(r: dict, i: int, total: int) -> str:
    ...
    # Image 1 style: "Link X/total - Status"
    line1 = f"{icon} Link {i}/{total} - {s_txt}"
    line2 = f"🔗 {r['link']}"
    line3 = f"📝 {title}"
    if members != "N/A":
        line3 += f"  👥 {members}"
    line4 = f"Type: {ltype}  |  Forward: {fwd_txt}"
...
def fmt_summary_header(total: int, active: int, expired: int, errors: int) -> str:
    return (
        ...
        f"Total: {total}  ✅ Active: {active}  ❌ Expired: {expired}  ⚠️ Error: {errors}\n"
        f"{'─'*34}\n\n"
    )
...
async def resolve_chat_smart(app, link: str):
    ...
    Private links: join_chat(hash) is the only reliable Pyrogram method.
    Public links: get_chat(username).
    """
    is_private, identifier = parse_link(link)
    ...
    if is_private:
        ...
        try:
            # join_chat with just the hash (no '+') — Pyrogram standard
            chat = await app.join_chat(identifier)
            return chat
        except UserAlreadyParticipant:
            # Already a member — find it in dialogs by iterating
            # Fastest: search_global or get_dialogs (limited but works)
            async for dialog in app.get_dialogs(limit=200):
                c = dialog.chat
                # Can't match by hash, so just return first supergroup/channel match
                # Better: store chat_id after first successful join
                pass
            # Fallback: try import_chat_invite_link to get chat object
            try:
                return await app.get_chat(f"https://t.me/+{identifier}")
            except Exception:
                raise Exception(
                    "Already a member of this chat. "
                    "Please use the public username/link of the group instead, "
                    "or check using the group's @username."
                )
        except InviteHashExpired:
        ...
        # Public username — straightforward
...
async def on_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "linkchecker":
        ...
        prog = await update.message.reply_text(
            f"⏳ Checking {total} link(s) in parallel...",
            disable_web_page_preview=True,
        )
        ...
        async def progress_cb(done: int, total: int, current_link: str):
            now = time.time()
            if now - last_prog_update[0] >= 2:
                last_prog_update[0] = now
                try:
                    await prog.edit_text(
                        f"⏳ Checking... {done}/{total}\n{current_link}",
                        disable_web_page_preview=True,
                    )
                except Exception:
                    pass

        # Use parallel checking — 8 concurrent for speed
        results = await check_links_bulk(links, uid, progress_cb=progress_cb, batch_size=8)
        ...
        # Build chunks (Telegram 4096 char limit)
        chunks_to_send = []
        body = ""
        for i, r in enumerate(results, 1):
            block = fmt_result(r, i, total)
