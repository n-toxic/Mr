...
async def check_via_http(link: str) -> dict:
    """
    Check ANY Telegram link via HTTP scraping.
    No account needed. Properly detects expired invite links.
    """
    result = {
    ...
    try:
        ...
        # ── Expired detection FIRST ──
        # Telegram shows these on expired/invalid invite pages
        expired_signals = [
            'tgme_page_additional">',      # shows "This invite link has expired"
        ...
        # Check if page has a join button (valid invite) vs no join button (expired)
        is_private, _ = parse_link(link)
        if is_private:
            # Private invite links: valid ones have tgme_action_button with "Join Group/Channel"
            has_join = bool(re.search(
                r'tgme_action_button|"Join\s+(Group|Channel|Chat)"|btn_join',
                html, re.I
            ))
            if not has_join:
                result["status"] = "expired"
        ...
        # ── Extract title ──
        m_title = re.search(r'<meta property="og:title"\s+content="([^"]+)"', html)
        m_desc  = re.search(r'<meta property="og:description"\s+content="([^"]+)"', html)

        title = m_title.group(1).strip() if m_title else ""
        desc  = m_desc.group(1).strip()  if m_desc  else ""

        # Generic titles = expired/invalid
        GENERIC = {
        ...
        if not title or title.lower() in GENERIC:
            result["status"] = "expired"
            result["title"]  = "Not found / Expired"
            return result

        # ── Extract member count ──
        mc = re.search(r"([\d\s,]+)\s*(members?|subscribers?|участник)", desc, re.I)
        members = mc.group(1).replace(",","").replace(" ","").strip() if mc else "N/A"

        result["status"]  = "active"
        result["title"]   = title
        result["members"] = members
        result["type"]    = "group" if is_private else "channel/group"
        result["forward"] = "unknown"
...
# ─────────────────────────────────────────
#  BULK LINK CHECKER
#  HTTP-first (no account, no FloodWait)
#  Pyrogram used only for forward-restriction info if account available
# ─────────────────────────────────────────

async def check_links_bulk(links: list[str], user_id: int,
                            progress_cb=None, batch_size: int = 15) -> list[dict]:
    """
    Check all links via HTTP scraping — fast, no account needed, no FloodWait.
    If Pyrogram account is available, it enriches forward-restriction data for private links.
    """
    from pyrogram import Client, raw as _raw

    session = get_session_path(user_id)

    # ── Step 1: HTTP check all links in parallel (15 concurrent) ──
    semaphore = asyncio.Semaphore(batch_size)
    results   = [None] * len(links)

    async def _http_check(i: int, lnk: str):
        async with semaphore:
            results[i] = await check_via_http(lnk)
            if progress_cb:
                done = sum(1 for x in results if x is not None)
                await progress_cb(done, len(links), lnk)

    tasks = [asyncio.create_task(_http_check(i, lnk)) for i, lnk in enumerate(links)]
    ...
    for i, r in enumerate(results):
        if r is None:
            results[i] = {
                "link": links[i], "status": "error", "type": "?",
                "title": "Timeout", "members": "N/A", "forward": "unknown",
            }

    # ── Step 2 (optional): Pyrogram to get forward-restriction for private links ──
    # Only runs if account is available AND there are active private links
    if session:
        private_active = [
            (i, r) for i, r in enumerate(results)
            if r["status"] == "active" and parse_link(r["link"])[0]
        ]
        if private_active:
            try:
                async with Client(session, api_id=API_ID, api_hash=API_HASH, no_updates=True) as app:
                    for i, r in private_active:
                        _, identifier = parse_link(r["link"])
                        try:
                            ...
                            if hasattr(inv, "chat"):
                                c = inv.chat
                                results[i]["forward"] = "OFF (Restricted)" if getattr(c, "noforwards", False) else "ON"
                                results[i]["members"] = getattr(c, "participants_count", r["members"])
                                results[i]["title"]   = getattr(c, "title", r["title"])
                            else:
                                results[i]["forward"] = "OFF (Restricted)" if getattr(inv, "noforwards", False) else "ON"
                                results[i]["members"] = getattr(inv, "participants_count", r["members"])
                                results[i]["title"]   = getattr(inv, "title", r["title"])
                            await asyncio.sleep(1.5)  # safe delay between MTProto calls
                        except Exception as e:
                            err = str(e)
                            if "FLOOD_WAIT" in err:
                                break  # stop enriching, HTTP results are still good
                            # Other error — keep HTTP result as-is
            except Exception:
                pass  # session error — HTTP results still valid
...
async def smart_forward_worker(
    ...
    try:
        ...
        async with Client(session, api_id=API_ID, api_hash=API_HASH, no_updates=True) as app:
            ...
            src_title = getattr(src_chat, "title", source_link)[:30]
            dst_title = getattr(dst_chat, "title", dest_link)[:30]

            # ── Get total message count estimate ──
            total_msgs = getattr(src_chat, "members_count", None)
            try:
                full = await app.get_chat(src_chat.id)
                total_msgs = getattr(full, "messages_count", None) or \
                             getattr(src_chat, "messages_count", None)
            except Exception:
                pass
            ...
            # Show initial status immediately (no "Initializing" hang)
            try:
                total_str = f"~{total_msgs}" if total_msgs else "?"
                await status_msg.edit_text(
                    f"⚡ LIVE FORWARD\n"
                    f"From : {src_title}\n"
                    f"To   : {dst_title}\n"
            ...
            async def send_one(msg) -> str:
                """Send one message. Returns 'ok' | 'skip' | 'fail'"""
                # Skip service messages
                if msg.service is not None:
                    return "skip"
                has_content = any([
                    msg.text, msg.photo, msg.video, msg.document,
                    msg.audio, msg.voice, msg.sticker, msg.animation,
                    msg.video_note, msg.caption,
                ])
                if not has_content:
                    return "skip"
                ...
                # Step 1: copy_message (fast, no download)
                try:
                    ...
                    if any(x in err for x in [
                        ...
                        "SERVICE_MESSAGE", "poll", "game", "invoice",
                        "cannot be copied", "CHAT_FORWARDS_RESTRICTED",
                    ]):
                        # Try download+upload for restricted
                        if "CHAT_FORWARDS_RESTRICTED" not in err and \
                           "cannot be copied" not in err:
                            return "skip"
                    if "FLOOD" in err:
                        raise FloodWait(int(re.search(r"\d+", err).group() or "30"))
                    ...
                    if msg.text:
                        ...
                        if tmp_path:
                            await app.send_video(
                                dst_chat.id, tmp_path, caption=cap,
                        ...
                        if tmp_path:
                            await app.send_audio(
                                ...
                                performer=getattr(msg.audio, "performer", "") or "",
                                title=getattr(msg.audio, "title", "") or "",
                            )
                            return "ok"
            ...
            # ── STREAMING FORWARD ──
            # Pyrogram get_chat_history gives newest first by default
            # We collect msg IDs first (lightweight), then process oldest→newest
            # This avoids the "reverse=True" param that doesn't exist in Pyrogram 2.x
            ...
            # Collect only message IDs (very fast, no media loading)
            msg_ids = []
            ...
            processed = 0
            # Process in batches of 200 IDs at a time to avoid memory issues
            BATCH = 200
            for batch_start in range(0, len(msg_ids), BATCH):
                if not forward_running.get(user_id):
                    break
                batch = msg_ids[batch_start:batch_start + BATCH]
                # Fetch full messages for this batch
                try:
                    msgs = await app.get_messages(src_chat.id, batch)
                ...
                for msg in msgs:
                    ...
                    if now - last_upd >= 4:
                        last_upd = now
                        elapsed  = now - start_t
                        m_t, s_t = divmod(int(elapsed), 60)
                        spd      = forwarded / elapsed if elapsed > 0 else 0

                        # ETA
                        if spd > 0 and processed < total_msgs:
                            remaining = (total_msgs - processed) / spd
                            rm, rs = divmod(int(remaining), 60)
                            rh, rm = divmod(rm, 60)
                            eta = f"{rh}h {rm}m" if rh else (f"{rm}m {rs}s" if rm else f"{rs}s")
                            eta_str = f"\n⏰ ETA      : ~{eta}"
                        else:
                            eta_str = ""
            ...
            done_stat = "COMPLETED ✅" if forward_running.get(user_id) else "STOPPED ⏹️"
...
async def _send_plain(chat_id: int, text: str, keyboard=None):
    """Send plain text message — no Markdown, no parse errors."""
    payload = {
        "chat_id": chat_id,
        "text": text,
    ...
    try:
        async with aiohttp.ClientSession() as s:
            resp = await s.post(f"{TG_API}/sendMessage", json=payload)
            return await resp.json()
    except Exception as e:
        logger.warning(f"_send_plain failed: {e}")
        return None
...
async def _run_link_check(links: list[str], uid: int, cid: int, prog):
    ...
    async def progress_cb(done: int, _total: int, current_link: str):
        now = time.time()
        if now - last_prog_update[0] >= 3:
            last_prog_update[0] = now
            try:
                await prog.edit_text(
                    f"⏳ Checking... {done}/{_total} links\nCurrent: {current_link[:50]}",
                    disable_web_page_preview=True,
                )
            except Exception:
                pass
    ...
    try:
        ...
        try:
            await prog.edit_text(f"❌ Check failed: {e}")
        except Exception:
            pass
        return
    ...
    try:
        await prog.edit_text(
            f"✅ Done! {total} links checked. Sending results...",
            disable_web_page_preview=True,
        )
    except Exception:
        pass

    # Build result text — PLAIN TEXT only (no markdown = no parse errors)
    def clean(txt) -> str:
        """Strip chars that could cause issues in plain text."""
        return str(txt or "N/A")[:80]

    def make_block(r: dict, i: int) -> str:
        s = r["status"]
        if s == "active":
            icon = "✅"
            label = "Working"
        elif s in ("expired", "invalid"):
            icon = "❌"
            label = "Expired"
        elif s == "need_account":
            icon = "🔐"
        ...
        fwd = str(r.get("forward", "unknown"))
        fwd_txt = "OFF ❌" if "OFF" in fwd else ("ON ✅" if fwd == "ON" else fwd)

        members = r.get("members", "N/A")
        mem_str = f"  👥 {members}" if members not in ("N/A", None, "") else ""

        return (
            f"\n{icon} Link {i}/{total} - {label}\n"
            f"🔗 {r['link']}\n"
            f"📝 {clean(r.get('title'))}{mem_str}\n"
            f"Forward: {fwd_txt}\n"
        )
    ...
    header = (
        f"✨ LINK CHECK RESULTS\n\n"
        f"Total links: {total}\n"
        f"Working: {active_n}  |  Expired: {expired_n}  |  Error: {error_n}\n"
        f"{'─' * 30}\n"
    )
    ...
    # Send all chunks as plain text — guaranteed, no parse errors
    back_btn = [[{"text": "🔙 Back to Menu", "callback_data": "back_main"}]]
    for idx, chunk in enumerate(chunks):
        kb = back_btn if idx == len(chunks) - 1 else None
        result = await _send_plain(cid, chunk, kb)
        if not result or not result.get("ok"):
            # Last resort: split into smaller pieces
            half = len(chunk) // 2
            await _send_plain(cid, chunk[:half])
            await asyncio.sleep(0.3)
            await _send_plain(cid, chunk[half:], kb)
        await asyncio.sleep(0.4)


async def on_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # Guard: ignore non-message updates and updates without text
    ...
    # Accept both text and caption (photos with caption containing links)
