    ...
    try:
        ...
        async def _check(app: Client):
            if is_private:
                ...
                # This checks invite link info WITHOUT joining — no side effects
                from pyrogram import raw as _raw
                try:
                    r = await app.invoke(
                        _raw.functions.messages.CheckChatInvite(hash=identifier)
                    )
                    # r is ChatInvite or ChatInviteAlready
                    # ChatInviteAlready  → already a member → has .chat
                    # ChatInvite         → not a member     → has .title, .participants_count
                    if hasattr(r, "chat"):
                        # Already member — extract from chat object
                        c = r.chat
                        ...
                        protected = getattr(c, "noforwards", False)
                        result["forward"] = "OFF (Restricted)" if protected else "ON"
                    else:
                        # Not a member — r is ChatInvite with invite info
                        result["status"]  = "active"
                        ...
                        protected = getattr(r, "request_needed", False)
                        result["forward"] = "unknown"
                    return
