...
async def resolve_chat_smart(app, link: str):
    """
    Resolve any link type for forwarding.
    Strategy:
    1. For private links: CheckChatInvite → if already member use chat.id, else join
    2. If link is expired but user is member, scan dialogs by title match
    3. For public: get_chat by username
    """
    from pyrogram import raw as _raw
    ...
    if is_private:
        ...
        # Step 1: CheckChatInvite — works even if not a member
        try:
            invite_result = await app.invoke(
                _raw.functions.messages.CheckChatInvite(hash=identifier)
            )
        except Exception as e:
            err = str(e)
            ...
            # Other errors — continue to try join

        # Step 2: If already a member (ChatInviteAlready has .chat)
        if invite_result is not None and hasattr(invite_result, "chat"):
            c = invite_result.chat
            raw_id = getattr(c, "id", None)
            if raw_id:
                # Try both plain id and -100 prefixed id
                for try_id in [raw_id, int(f"-100{raw_id}")]:
                    try:
                        return await app.get_chat(try_id)
                    except Exception:
                        pass

        # Step 3: If link expired, try to find this chat in dialogs by title
        if invite_expired:
            # Get title from the expired invite result if we have it
            title_hint = None
            if invite_result and hasattr(invite_result, "title"):
                title_hint = invite_result.title
            # Scan dialogs to find matching chat
            async for dialog in app.get_dialogs(limit=500):
                c = dialog.chat
                if title_hint and getattr(c, "title", "") == title_hint:
                    return c
            raise Exception(
                "Invite link has expired and could not find this chat in your joined chats.\n"
                "Please use the group's public @username link instead."
            )

        # Step 4: Not a member — join
        try:
            return await app.join_chat(identifier)
        except UserAlreadyParticipant:
            # join says already participant — scan dialogs
            if invite_result and hasattr(invite_result, "chat"):
                raw_id = invite_result.chat.id
                for try_id in [raw_id, int(f"-100{raw_id}")]:
                    try:
                        return await app.get_chat(try_id)
                    except Exception:
            ...
            async for dialog in app.get_dialogs(limit=200):
                return dialog.chat  # return first one as last resort
            raise Exception("Already a member but could not fetch chat.")
        except Exception as e:
            err = str(e)
            if "INVITE_HASH_EXPIRED" in err:
        ...
        async with Client(session, api_id=API_ID, api_hash=API_HASH) as app:
            ...
            for msg in messages:
                if not forward_running.get(user_id):
                    break
                try:
                    # copy_message = re-upload with original quality, no "Forwarded from" tag
                    # Works even when forward is restricted (copies content, not forwards)
                    ...
                    err = str(ex)
                    # Skip service messages, polls, etc. that can't be copied
                    if any(x in err for x in ["MEDIA_EMPTY", "MESSAGE_ID_INVALID",
                                               "poll", "game", "invoice"]):
                        skipped += 1
                    else:
                        failed += 1
