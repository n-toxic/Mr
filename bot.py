...
async def resolve_chat_smart(app, link: str):
    ...
    Private invite hash → raw MTProto CheckChatInvite to get chat_id, then join if needed.
    Public username → get_chat directly.
    """
    from pyrogram import raw as _raw
    ...
    if is_private:
        # Step 1: Check invite to get chat info (works even without being member)
        try:
            r = await app.invoke(_raw.functions.messages.CheckChatInvite(hash=identifier))
        except Exception as e:
            err = str(e)
            if "INVITE_HASH_EXPIRED" in err:
        ...
        if hasattr(r, "chat"):
            # Already a member — r.chat has the chat object with id
            chat_id = r.chat.id
            # Make it negative for supergroups/channels if needed
            try:
                return await app.get_chat(chat_id)
            except Exception:
                # Try with -100 prefix for supergroups
                try:
                    return await app.get_chat(int(f"-100{chat_id}"))
                except Exception:
                    pass
            # If get_chat fails, join anyway
        
        # Not a member (or get_chat failed) — join the chat
        try:
            chat = await app.join_chat(identifier)
            return chat
        except UserAlreadyParticipant:
            # join failed but already member — try by chat_id from CheckChatInvite
            if hasattr(r, "chat") and hasattr(r.chat, "id"):
                try:
                    return await app.get_chat(r.chat.id)
                except Exception:
                    try:
                        return await app.get_chat(int(f"-100{r.chat.id}"))
                    except Exception:
                        pass
            raise Exception("Already a member but could not fetch chat. Use public @username instead.")
