...
async def check_via_http(link: str) -> dict:
    """
    Check ANY Telegram link (public or private) via HTTP scraping.
    No account needed. No rate limits. Works for t.me/+hash too.
    Telegram's web preview returns og:title and og:description for valid links.
    """
    result = {
    ...
    try:
        ...
        # og:title = group/channel name
        m_title = re.search(r'<meta property="og:title"\s+content="([^"]+)"', html)
        # og:description = "X members" or "X subscribers"
        m_desc  = re.search(r'<meta property="og:description"\s+content="([^"]+)"', html)

        title = m_title.group(1).strip() if m_title else ""
        desc  = m_desc.group(1).strip()  if m_desc  else ""

        # If title is generic "Telegram" or empty → expired/invalid
        if not title or title.lower() in ("telegram", "telegram messenger"):
            # Check for explicit "link is invalid" or "expired"
            if re.search(r"(link.*invalid|invalid.*link|link.*expired|expired)", html, re.I):
                result["status"] = "expired"
                result["title"]  = "Invite link expired"
            else:
                result["status"] = "expired"
        ...
        if mc:
            members = mc.group(1).replace(",", "").replace(" ", "").strip()
        else:
            members = "N/A"

        # Detect type from URL
        is_private, _ = parse_link(link)
        ltype = "group" if is_private else "channel/group"

        result["status"]  = "active"
        result["title"]   = title
        result["members"] = members
        result["type"]    = ltype
        result["forward"] = "unknown"  # HTTP can't tell forward restriction
...
async def smart_forward_worker(
    ...
    try:
        ...
        async with Client(session, api_id=API_ID, api_hash=API_HASH, no_updates=True) as app:
            ...
            # ── STREAMING FORWARD — no pre-loading all messages ──
            # get_chat_history streams oldest-first with reverse=True
            processed = 0
            async for msg in app.get_chat_history(src_chat.id, reverse=True):
                ...
                try:
                    ...
                    # Update status to show flood wait
                    try:
                        await status_msg.edit_text(
                            f"⚡ LIVE FORWARD\n"
                            f"From : {src_title}\n"
                ...
                if now - last_upd >= 4:
                    ...
                    # Time estimate
                    if spd > 0 and total_msgs and processed < total_msgs:
